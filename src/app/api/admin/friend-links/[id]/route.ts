...
import { updateFriendLinkSchema } from '../schema'
import { deleteFriendLink } from './delete'
import { updateFriendLink } from './patch'
...
export async function DELETE(_request: NextRequest, { params }: { params: Promise<{ id: string }> }): Promise<NextResponse<string | SingleDeleteData>> {
  ...
  return await deleteFriendLink(Number(id)).then(() => {
...
export async function PATCH(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  const input = await parsePatchBody(request, updateFriendLinkSchema)
  const { id } = await params
  if (input instanceof YumeError) {
    return NextResponse.json(input.toJSON())
  }

  const friendLink = await updateFriendLink(input, Number(id))
  return createSingleEntityResponse(friendLink)
}
